...
export class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/mka/icons/svg/item-grant.svg",
      title: game.i18n.localize("MKA.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("MKA.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.data.value.added || configMode ) {
      return this.data.configuration.items.reduce((html, uuid) => html + mka.utils.linkForUuid(uuid), "");
    }
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.mka.sourceId;
    }
    this.updateSource({"value.added": updates});
  }
...
export class ItemGrantConfig extends AdvancementConfig {
   ...
   * @returns {Promise<ItemMKA>}  The promise for the updated par. Item which resolves after the application re-renders
   * @private
   */
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      return ui.notifications.warn(game.i18n.localize("MKA.AdvancementItemGrantRecursiveWarning"));
    }

    // Abort if this uuid exists already
    if ( existingItems.includes(item.uuid) ) {
      return ui.notifications.warn(game.i18n.localize("MKA.AdvancementItemGrantDuplicateWarning"));
    }
...
export class ItemGrantFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/mka/templates/advancement/item-grant-flow.hbs"
    });
  }
  ...
  async getData() {
    ...
    const added = this.retainedData?.items.map(i => foundry.utils.getProperty(i, "flags.mka.sourceId"))
  ...
  async _updateObject(event, formData) {
    const retainedData = this.retainedData?.items.reduce((obj, i) => {
      obj[foundry.utils.getProperty(i, "flags.mka.sourceId")] = i;
      return obj;
    }, {});
