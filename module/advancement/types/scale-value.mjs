...
export class ScaleValueAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 60,
      icon: "systems/mka/icons/svg/scale-value.svg",
      title: game.i18n.localize("MKA.AdvancementScaleValueTitle"),
      hint: game.i18n.localize("MKA.AdvancementScaleValueHint"),
      multiLevel: true,
   ...
   */
  static TYPES = {
    string: "MKA.AdvancementScaleValueTypeString",
    number: "MKA.AdvancementScaleValueTypeNumber",
    dice: "MKA.AdvancementScaleValueTypeDice",
    distance: "MKA.AdvancementScaleValueTypeDistance"
  };
  ...
  formatValue(level) {
    ...
    const value = this.valueForLevel(level);
    if ( value == null ) return null;
    return `${value.value} ${CONFIG.MKA.movementUnits[this.data.configuration.distance.units]}`;
  }
...
export class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mka", "advancement", "scale-value", "two-column"],
      template: "systems/mka/templates/advancement/scale-value-config.hbs",
      width: 540
    });
  }
  ...
  getData() {
    ...
    data.typeHint = game.i18n.localize(`MKA.AdvancementScaleValueTypeHint${config.type.capitalize()}`);
    data.types =
      Object.fromEntries(
    ...
    data.movementUnits = CONFIG.MKA.movementUnits;
    return data;
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.MKA.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (formData["data.configuration.type"] !== this.advancement.data.configuration.type) ) {
      ...
      for ( const l of Array.fromRange(CONFIG.MKA.maxLevel, 1) ) {
...
export class ScaleValueFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/mka/templates/advancement/scale-value-flow.hbs"
    });
  }
