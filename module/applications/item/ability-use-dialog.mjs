...
export default class AbilityUseDialog extends Dialog {
  ...
  static async create(item) {
    ...
    const data = {
      item: item,
      title: game.i18n.format("MKA.AbilityUseHint", {type: game.i18n.localize(`MKA.ItemType${item.type.capitalize()}`), name: item.name}),
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "MKA.AbilityUseChargedHint" : "MKA.AbilityUseRechargeHint", {
        type: game.i18n.localize(`MKA.ItemType${item.type.capitalize()}`)
      });
    }
