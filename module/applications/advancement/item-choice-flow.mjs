...
export default class ItemChoiceFlow extends ItemGrantFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/mka/templates/advancement/item-choice-flow.hbs"
    });
  }
  ...
  async getContext() {
    this.selected ??= new Set(
      this.retainedData?.items.map(i => foundry.utils.getProperty(i, "flags.mka.sourceId"))
    ...
    if ( !this.dropped ) {
      ...
      for ( const data of this.retainedData?.items ?? [] ) {
        const uuid = foundry.utils.getProperty(data, "flags.mka.sourceId");
  ...
  async _onDrop(event) {
    ...
    for ( const [level, data] of Object.entries(this.advancement.value.added ?? {}) ) {
      ...
      if ( Object.values(data).includes(item.uuid) ) {
        return ui.notifications.error(game.i18n.localize("MKA.AdvancementItemChoicePreviouslyChosenWarning"));
      }
    }
    ...
    if ( (this.advancement.configuration.type === "spell") && spellLevel === "available" ) {
      ...
      if ( item.system.level > maxSlot ) return ui.notifications.error(game.i18n.format(
        "MKA.AdvancementItemChoiceSpellLevelAvailableWarning", { level: CONFIG.MKA.spellLevels[maxSlot] }
      ));
    }
  ...
  _maxSpellSlotLevel() {
    ...
    if ( spellcasting ) {
      ...
      ActorMKA.computeClassProgression(progression, this.advancement.item, { spellcasting });
      ActorMKA.prepareSpellcastingSlots(spells, spellcasting.type, progression);
    }
