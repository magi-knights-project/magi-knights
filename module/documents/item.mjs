...
export default class ItemMKA extends Item {
   ...
   * @property {string} type              Spellcasting type as defined in ``CONFIG.MKA.spellcastingTypes`.
  ...
  get spellcasting() {
    ...
    if ( CONFIG.MKA.spellcastingTypes[finalSC.progression] ) finalSC.type = finalSC.progression;
    else finalSC.type = Object.entries(CONFIG.MKA.spellcastingTypes).find(([type, data]) => {
  ...
  prepareFinalAttributes() {
    ...
    if ( "actionType" in this.system ) {
      this.labels.abilityCheck = game.i18n.format("MKA.AbilityPromptTitle", {
        ability: CONFIG.MKA.abilities[this.system.ability]?.label ?? ""
      });
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.MKA.abilities[save.ability]?.label ?? "";
  ...
  prepareMaxUses() {
    ...
    if ( this.isOwned && !Number.isNumeric(max) ) {
      const property = game.i18n.localize("MKA.UsesMax");
      try {
        const rollData = this.getRollData({ deterministic: true });
        max = Roll.safeEval(this.replaceFormulaData(max, rollData, { property }));
      } catch(e) {
        const message = game.i18n.format("MKA.FormulaMalformedError", { property, name: this.name });
  ...
  prepareDurationValue() {
    ...
    if ( this.isOwned && !Number.isNumeric(value) ) {
      const property = game.i18n.localize("MKA.Duration");
      try {
        const rollData = this.getRollData({ deterministic: true });
    ...
    this.labels.duration = [duration.value, CONFIG.MKA.timePeriods[duration.units]].filterJoin(" ");
  }
  ...
  replaceFormulaData(formula, data, { property }) {
    ...
    if ( (missingReferences.size > 0) && this.actor ) {
      ...
      const message = game.i18n.format("MKA.FormulaMissingReferenceWarn", {
  ...
  async use(config={}, options={}) {
    ...
    options = foundry.utils.mergeObject({
      ...
      createMessage: true,
      "flags.mka.use": {type: this.type, itemId: this.id, itemUuid: this.uuid}
    }, options);
    ...
    if ( isSpell ) foundry.utils.mergeObject(options.flags, {"mka.use.spellLevel": item.system.level});
    ...
    if ( config.createMeasuredTemplate ) {
      try {
        ...
        Hooks.onError("ItemMKA#use", err, {
          msg: game.i18n.localize("MKA.PlaceTemplateError"),
          log: "error",
          notify: "error"
        });
      }
  ...
  async displayCard(options={}) {
    ...
    if ( (this.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["mka.itemData"] = templateData.item.toObject();
    }
  ...
  async rollAttack(options={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      messageData: {
        "flags.mka.roll": {type: "attack", itemId: this.id, itemUuid: this.uuid},
  ...
  async rollDamage({critical, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const messageData = {
      "flags.mka.roll": {type: "damage", itemId: this.id, itemUuid: this.uuid},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
  ...
  async rollFormula({spellLevel}={}) {
    ...
    if ( rollConfig.chatMessage ) {
      roll.toMessage({
        ...
        messageData: {"flags.mka.roll": {type: "other", itemId: this.id, itemUuid: this.uuid}}
      });
    }
  ...
  static async _onChatCardAction(event) {
    ...
    switch ( action ) {
      ...
      case "placeTemplate":
        try {
          await mka.canvas.AbilityTemplate.fromItem(item)?.drawPreview();
        } catch(err) {
          Hooks.onError("ItemMKA._onChatCardAction", err, {
            msg: game.i18n.localize("MKA.PlaceTemplateError"),
            log: "error",
            notify: "error"
          });
   ...
   * @returns {Promise<AdvancementConfig>|ItemMKA}  Promise for advancement config for new advancement if local
  ...
  createAdvancement(type, data={}, { showConfig=true, source=false }={}) {
    ...
    const Advancement = CONFIG.MKA.advancementTypes[type];
    if ( !Advancement ) throw new Error(`${type} not found in CONFIG.MKA.advancementTypes`);
   ...
   * @param {object} [options={}]
   * @param {boolean} [options.source=false]  Should a source-only update be performed?
   * @returns {Promise<ItemMKA>|ItemMKA}        This item with the changes applied, promised if source is `false`.
   */
   ...
   * @param {object} [options={}]
   * @param {boolean} [options.source=false]  Should a source-only update be performed?
   * @returns {Promise<ItemMKA>|ItemMKA}        This item with the changes applied.
   */
   ...
   * @param {boolean} [options.showConfig=true]     Should the new advancement's configuration application be shown?
   * @param {boolean} [options.source=false]        Should a source-only update be performed?
   * @returns {Promise<AdvancementConfig>|ItemMKA}   Promise for advancement config for duplicate advancement if source
   ...
   * @param {ItemMKA|object} spell     The spell or item data to be made into a scroll
  ...
  static async createScrollFromSpell(spell, options={}) {
    ...
    + `<hr><h3>${itemData.name} (${game.i18n.format("MKA.LevelNumber", {level})})</h3>`
    + (components.concentration ? `<p><em>${game.i18n.localize("MKA.ScrollRequiresConcentration")}</em></p>` : "")
    + `<hr>${description.value}<hr>`
    + `<h3>${game.i18n.localize("MKA.ScrollDetails")}</h3><hr>${scrollDetails}`;
     ...
     * @function mka.createScrollFromSpell
     * @memberof hookEvents
     * @param {ItemMKA|object} spell       The spell or item data to be made into a scroll.
     * @param {object} spellScrollData    The final item data used to make the scroll.
     */
    Hooks.callAll("mka.createScrollFromSpell", spell, spellScrollData);
  ...
  getCriticalThreshold() {
    foundry.utils.logCompatibilityWarning(
      "ItemMKA#getCriticalThreshold has been replaced with the ItemMKA#criticalThreshold getter.",
